...
StlFile::StlFile()
{
	...
	itsShadeRed = 182;
	itsShadeGreen = 182;
	itsShadeBlue = 182;//Material 속성설정
...
void StlFile::open(char* fileName, char* FileTitle)
{
	strcpy(STLFileName, fileName);
	strcpy(STLFileTitle, FileTitle);
  initialize(fileName);
  allocate();
  readData(0, 1);
  file.close();
...
void StlFile::initialize(char* fileName) {
  stats.numFacets = 0;
  stats.numPoints = 0;
  stats.surface = -1.0;
  stats.volume = -1.0;
...
void StlFile::readData(int firstFacet, int first)
{
  ...
  for (int i = firstFacet; i < stats.numFacets; i++)
  {
    ...
    if (first)
	{
	    ...
	    float xDiff = fabs(facet.vector[0].x - facet.vector[1].x);
	    float yDiff = fabs(facet.vector[0].y - facet.vector[1].y);
	    float zDiff = fabs(facet.vector[0].z - facet.vector[1].z);
